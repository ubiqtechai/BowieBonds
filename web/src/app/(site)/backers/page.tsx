...
const BENEFITS = [
  ["Copyright-backed", "Your funds aren't just a promise. The artiste grants an exclusive copyright license on that specific promoted work — and only that work — to the backer pool. If the artiste defaults, you retain the license on the work."],
  ...
  ["Capped returns, then copyright reverts", "Every drop has a return cap (e.g. 1.5x or 2.0x). When you're made whole, the copyright automatically reverts to the artiste. Clean exit."],
...
const RISKS = [
  ...
  ["Artiste default risk", "If an artiste stops making payments, you retain the copyright license on the specific promoted work — but monetising it independently may be difficult. Enforcement is through copyright retention and platform exclusion."],
  ["This is not an investment product", "ZiggyDust facilitates copyright licensing arrangements — scoped to a single promoted work per drop — between artistes and backers. This is a commercial IP transaction — not a loan, not equity, not a security. Consult your own legal and financial advisors."],
];
        ...
        <div className="border-2 border-red p-5 lg:p-7 mt-5">
          {RISKS.map(([t, d], i) => (
            <div key={t} className={`${i < RISKS.length - 1 ? "mb-4 pb-4 border-b border-sand" : ""}`}>
              ...
              <div className="text-sm text-ink-mid leading-relaxed">{d}</div>
            </div>
          ))}
        </div>
        <Link href="/signup">
