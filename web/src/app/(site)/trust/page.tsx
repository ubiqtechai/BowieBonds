...
const LAYERS = [
  {
    ...
    why: "Promises are easy. Putting your intellectual property on the line is not. When an artist licenses the copyright on a specific promoted work to the backer pool, they're staking the one thing that matters most — that creative work. The license covers only that one work, not the channel or catalogue. This is the enforcement mechanism: if an artist defaults, backers retain the exclusive copyright license on that work.",
    how: "Artist grants an exclusive license on the specific promoted work's copyright — and only that work — to the backer pool at drop creation. License persists for the campaign period. Reverts automatically when backers are made whole (cap hit or license period expiry). On default, backers retain the license on the work.",
  },
  {
  ...
  },
  {
    ...
    why: "Self-reported numbers are meaningless. If an artist says they have 50K subscribers, you should be able to verify that. If they say their daily revenue is $20, you should see the API data.",
    how: "Artist connects YouTube via OAuth during signup. We pull subscriber count, total views, monthly views, channel age. Revenue data tracked daily. All read-only — we can't touch their channel.",
  },
  {
    t: "The Lockbox",
    ...
    how: "Funds go in from artist co-pay and backers. Funds come out only as Google Ads deployments. Every deployment generates a receipt with Google invoice reference, impressions, views, CTR.",
  },
  {
    t: "20% artist co-pay",
    why: "If someone asks you to fund their promotion but won't put their own money in, that's a red flag. The 20% minimum co-pay ensures the artist has genuine skin in the game — and it's first-in, first-loss.",
    how: "Enforced at drop creation. Cannot submit with less than 20%. Displayed prominently on every drop. Backers can see exactly how much the artist committed.",
  },
  {
    t: "Track records",
            ...
            <div className="flex-1">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div>
                  ...
                  <div className={`text-sm leading-relaxed ${i % 2 === 1 ? "text-ink-light" : "text-ink-mid"}`}>{s.why}</div>
                </div>
                <div>
                  <Mono className="text-[9px] text-amber uppercase tracking-widest block mb-2">How</Mono>
                  <div className={`text-sm leading-relaxed ${i % 2 === 1 ? "text-ink-light" : "text-ink-mid"}`}>{s.how}</div>
                </div>
              </div>
            </div>
          </div>
